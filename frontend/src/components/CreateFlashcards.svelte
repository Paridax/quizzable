...
<div class="xl:px-72">
    ...
    <div class="flex items-right">
        <button class="btn variant-soft-secondary ml-auto" on:click|preventDefault={() => {
    ...
    <div class="gap-5 mb-5">
        {#each data.cards as card, cardNum}
            <form>
                <div class="card card w-full p-5 my-5">
                    <div class="flex justify-between">
                        <h3 class="h3 font-bold">{cardNum + 1}</h3>
                        <div class="flex justify-between gap-5">
                        <button class="btn variant-filled-secondary" on:click|preventDefault={() => {
                            console.log("move up");
                            if (cardNum != 0) {
                            ...
                            data.cards = [...data.cards];
                            syncedWithServer = false;
                        }} disabled={cardNum == 0}>Move Up</button>
                        <button class="btn variant-filled-secondary" on:click|preventDefault={() => {
                            ...
                            data.cards = [...data.cards];
                            syncedWithServer = false;
                        }} disabled={cardNum == data.cards.length - 1}>Move Down</button>
                        <button class="btn variant-soft-error w-[2.625rem]" on:click|preventDefault={() => {
                            deleteCard(cardNum);
                        }}>✖</button>
                        </div>
                    </div>
        ...
        {:else}
            <div class="w-full h-24 flex items-center justify-center mb-10 pb-2.5">
                <h1 class="font-semibold text-surface-500-400-token h-3">There are no cards in this set...</h1>
            </div>
        {/each}
        <form>

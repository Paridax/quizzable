<script lang="ts">
	...
	$: visibility =
		set.visibility === 'public' ? 'Public' : set.visibility === 'unlisted' ? 'Unlisted' : 'Private';

	export let authorUsername = '';

	onMount(async () => {
		const author: User = await pb
			.collection('publicUsers')
			.getOne(set.author, { $autoCancel: false });
		console.log(author);
...
>
	<div>
		...
		{#if authorUsername}
			<p class="text-sm p-dark">
				by <a on:click|stopPropagation href={`/user/${authorUsername}`}>@{authorUsername}</a>
			</p>
		{/if}
		<p class="w-full line-clamp-2 break-words text-surface-600-300-token text-sm mt-2.5">
			{set.description ? set.description : 'No description.'}
		</p>
	</div>

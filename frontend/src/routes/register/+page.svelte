...
<main>
    <div class="max-w-md mx-auto my-28">
        <div class=" p-5 card w-full">
            ...
            <form class="flex flex-col gap-2.5 w-full" method="POST" on:submit={() => {loading = true;}} use:enhance={() => {
                return async ({result}) => {
                    console.log(result);
                    loading = false;
                    errors = {
                ...
                <label class="label">
                    <span class="text-surface-600-300-token">Username (a-z, 0-9)<span class="text-error-500-400-token">*</span></span>
                ...
                <label class="label">
                    <span class="text-surface-600-300-token">Email Address<span class="text-error-500-400-token">*</span></span>
                ...
                <label class="label">
                    <span class="text-surface-600-300-token">Confirm Password<span class="text-error-500-400-token">*</span></span>
                    <input name="passwordConfirm" type="password" class="input" bind:value={passwordConfirm} />
                ...
                <p class="text-sm text-surface-700-200-token text-center mt-2.5">Already have an account? <a href="/login" class="text-surface-700-200-token hover:text-surface-800-100-token font-semibold">Login</a></p>
            </form>
        </div>
        <p class="text-xs text-surface-500-400-token text-center mt-2.5 w-3/4 mx-auto">By creating an account on Quizzable, you agree to our <a href="/legal/terms" class="text-surface-500-400-token hover:text-surface-600-300-token underline">Terms of Service</a> and <a href="/legal/privacy" class="text-surface-500-400-token hover:text-surface-600-300-token underline">Privacy Policy</a>.</p>
	</div>
</main>

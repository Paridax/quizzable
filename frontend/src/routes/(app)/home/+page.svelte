<script lang="ts">
	...
	import QuizzableCard from '$lib/components/QuizzableCard.svelte';
	import type { PublicSet as Set } from '$lib/types';
...
{#if $currentUser}
	<div class="xl:max-w-5xl container h-full mx-auto px-5">
		<div class="space-y-5 py-10">
			<h2 class="h2 font-bold w-full text-center">
				Welcome to Quizzable, {$currentUser?.displayName}!
			</h2>
		</div>
		<div class="mt-10 flex flex-col gap-5">
			...
			<div>
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 my-5">
					...
					<a
						href="/create"
						class="hover:variant-filled-primary group flex flex-col items-center justify-center gap-2 card py-24 card-hover relative"
					>
						<svg
						...
						<p
							class="absolute left-1/2 top-1/2 -translate-x-1/2 translate-y-5 group-hover:translate-y-4 transition-all ease-out group-hover:opacity-100 opacity-0"
						>
							Create
						</p>
					</a>

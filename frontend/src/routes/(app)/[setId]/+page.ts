...
export const load = async ({ params }) => {
	...
	const author = (await pb
		...
		.getOne(typeof set.author === 'string' ? set.author : set.author.id)
		.catch((e) => {
			console.log(e);
			return {
				id: '0',
	...
	const soloQuiz = await pb
		.collection('soloQuizzes')
		.getFirstListItem(
			`user = "${pb.authStore.model?.id}" && quizzable = "${set.id}" && completedQuiz = false`
		)
		.catch(() => undefined);
	...
	console.log(soloQuiz);

	const userLikesThis = pb.authStore.model
		? await pb
				.collection('likes')
				.getFirstListItem(`user = "${pb.authStore.model.id}" && quizzable = "${params.setId}"`)
				.then(() => true)
				.catch(() => false)
		: false;
	...
	if (pb.authStore.model) {
		...
		console.log(
			`user = "${pb.authStore.model.id}" && quizzable = "${params.setId} && created > "${dateCutoffString}"`
		);
		await pb
			.collection('quizzableViews')
			.getFirstListItem(
				`user = "${pb.authStore.model.id}" && quizzable = "${params.setId}" && created > "${dateCutoffString}"`
			)
			.catch(() => {
	...
	return {
		...
		soloQuiz: soloQuiz ? soloQuiz.id : null
	};
};

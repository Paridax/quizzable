<script lang="ts">
	...
	function startTheQuiz() {
		if (typeof data.soloQuiz === 'string') {
			goto(`/quiz/${data.soloQuiz}`);
		} else {
			goto(`/${data.set.id}/quiz`);
		}
	}
</script>
...
<div class="small-container mx-auto py-20 px-5 md:px-0 mt-36 md:mt-48 lg:mt-96 flex flex-col gap-2">
	<div class="w-full flex justify-between items-end gap-5">
		...
		<div class="shrink-0 hidden sm:flex items-center gap-5">
			<LikeButton
				...
				bind:liked={userLikesThis}
				bind:likeCount={data.set.likes}
			/>
	...
	{#if typeof data.set.author !== 'string'}
		<div class="font-semibold h4 text-lg flex gap-1 items-center w-full overflow-hidden mr-20">
			...
			<a
				href={`/user/${data.set.author.username}`}
				class="block p font-semibold flex gap-1 items-center max-w-full"
			>
				<span class="text-surface-900-50-token hover:underline truncate"
					>{data.set.author.displayName}</span
				>
				{#if data.set.author.verifiedCreator}
					<a on:click|stopPropagation href="/help/verified-creator" use:popup={popupHover}>
						<svg
							...
							viewBox="0 0 24 24"
							fill="currentColor"
							class="w-6 h-6 text-primary-500 [&>*]:pointer-events-none"
						>
							<path
								fill-rule="evenodd"
					...
					<div
						class="card p-2 px-4 text-base variant-filled-primary z-20 shadow-xl"
					...
					<p class="text-surface-500-400-token font-semibold text-sm ml-1 mr-10 shrink-0">
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
		...
		<div class="card p-5 px-7">
			<span class="text-surface-500-400-token font-bold">1.</span>
			{data.cards[0].termOrQuestion}
		</div>
		<div class="card p-5 px-7">
			<span class="text-surface-500-400-token font-bold">2.</span>
			{data.cards[1].termOrQuestion}
		</div>
		{#if data.cards.length > 7}
			<div class="card p-5 px-7">
				<span class="text-surface-500-400-token font-bold">3.</span>
				{data.cards[2].termOrQuestion}
			</div>
		{/if}
	</div>
	<hr class="my-7" />
	<div
		class="grid gap-5"
		class:grid-cols-2={typeof data.soloQuiz === 'string'}
		class:grid-cols-1={typeof data.soloQuiz !== 'string'}
	>
		<button
			class="col-span-1 card variant-filled-primary w-full flex items-center justify-center py-12 card-hover transition-all hover:bg-gradient-to-br hover:to-warning from-primary to-primary active:scale-[97%]"
			on:click={startTheQuiz}
		>
			<h3 class="h3 font-semibold">
				{typeof data.soloQuiz === 'string' ? 'Continue the Quiz' : 'Start the Quiz'}
			</h3>
		</button>
		{#if typeof data.soloQuiz === 'string'}
			<button
				class="col-span-1 card variant-soft-secondary hover:variant-soft-primary w-full flex items-center justify-center py-12 card-hover transition-all hover:bg-gradient-to-br hover:to-warning from-primary to-primary active:scale-[97%] active:variant-filled-primary"
				on:click={() => goto(`/${data.set.id}/quiz`)}
			>
				<h3 class="h3 font-semibold">Start Over</h3>
			</button>
		{/if}
	</div>
</div>
...
<style>
	#heading {
		...
		background-image: radial-gradient(at 36% 75%, hsla(347, 91%, 72%, 1) 0px, transparent 50%),
